            ...
            float[] verticies =
{
        ...
        public void InitElement3D(Element3D element)
        {
            ...
            foreach(Material material in element.Model.Materials)
            {
                ...
                //material.Propeterys.Add("normal_id", this.InitElement3DTexture(material.NormalPath));
                ...
                //float[] colorf = this.GetColorArray(Color.White);
                //int cbo = gl.GenBuffer(1);
                //gl.BindBuffer(OpenGL.ArrayBuffer, cbo);
                //gl.BufferData(OpenGL.ArrayBuffer, colorf.Length * sizeof(float), colorf, OpenGL.DynamicDraw);
                //material.Propeterys.Add("cbo", cbo);
        ...
        public void SetCamera(Camera camera)
        {
            ...
            if(camera.Type == CameraType.Ortho)
            {
                ...
                v_mat = mat4.LookAt(new vec3(camera.Location.X, camera.Location.Y, camera.Location.Z), new vec3(0f, 0f, 0f), new vec3(0f, 1f, 0f));
            }
            else
            {
            ...
            float[] textCoordsf =
{
        ...
        public void DrawElement3D(Element3D element)
        {
            ...
            {
                gl.UseProgram(this.ShaderPrograms["DiffuseShader"].ProgramID);
                gl.UniformMatrix4fv(gl.GetUniformLocation(this.ShaderPrograms["DiffuseShader"].ProgramID, "mvp"), 1, false, mvp.ToArray());

                gl.ActiveTexture(OpenGL.Texture0);
                gl.BindTexture(OpenGL.Texture2D, (int)material.Propeterys["tex_id"]);
                gl.Uniform1I(gl.GetUniformLocation(this.ShaderPrograms["DiffuseShader"].ProgramID, "textureSampler"), 0);
