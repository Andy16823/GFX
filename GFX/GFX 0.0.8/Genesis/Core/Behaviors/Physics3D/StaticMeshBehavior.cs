...
namespace Genesis.Core.Behaviors.Physics3D
{
    public class StaticMeshBehavior : Physics.PhysicsBehavior
    {
        ...
        public void CreateRigidBody(Physics.PhysicHandler handler, float mass)
        {
            if (this.Parent.GetType() == typeof(Element3D))
            {
                Element3D element = (Element3D)this.Parent;

                object[] mesh = element.Model.GetIndexMesh();
                int[] indicies = (int[])mesh[0];
                float[] verticies = (float[])mesh[1];
